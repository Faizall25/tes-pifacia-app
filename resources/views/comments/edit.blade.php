        ...
        <div class="card-body">
            <form action="{{ route('comments.update', $comment) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="metadata" id="metadata" class="form-control">{{ old('metadata', json_encode($comment->metadata)) }}</textarea>
                    @error('metadata')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <select name="task_id" id="task_id" class="form-control select2" required>
                        @foreach ($tasks as $task)
                            <option value="{{ $task->id }}" {{ old('task_id', $comment->task_id) == $task->id ? 'selected' : '' }}>{{ $task->title }}</option>
                        @endforeach
                    </select>
                    @error('task_id')
                        <small class="text-danger">{{ $message }}</small>
