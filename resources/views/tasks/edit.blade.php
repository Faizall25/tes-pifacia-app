            ...
            <form action="{{ route('tasks.update', $task) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select name="is_completed" id="is_completed" class="form-control">
                        <option value="0" {{ old('is_completed', $task->is_completed) === 0 ? 'selected' : '' }}>No</option>
                        <option value="1" {{ old('is_completed', $task->is_completed) ? 'selected' : '' }}>Yes</option>
                    </select>
                    @error('is_completed')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea name="metadata" id="metadata" class="form-control">{{ old('metadata', json_encode($task->metadata)) }}</textarea>
                    @error('metadata')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
